...
const CHATBAR_WIDTH   = 260; // px → left sidebar inner edge
const PROMPTBAR_WIDTH = 320; // px → right sidebar inner edge
...
export const CloseSidebarButton: React.FC<Props> = ({ side, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className="z-40 h-8 w-8 fixed flex items-center justify-center rounded-md bg-gray-100 text-gray-600 hover:text-white hover:bg-teal-600 shadow"
  ...
  return (
    <button
      ...
      className={`z-40 h-8 w-8 fixed flex items-center justify-center
                  ...
                  bg-gray-100 text-gray-600 hover:text-white hover:bg-teal-600 shadow`}
      style={{ top: HEADER_HEIGHT, ...offset }}
