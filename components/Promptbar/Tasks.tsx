...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  // Timer check effect
  useEffect(() => {
    if (task.completionStatus === 'complete' || !task.timerEnd) {
      if (isTimerExpired) setIsTimerExpired(false); setTimeRemaining(''); return;
    }
    ...
    const checkTimer = () => {
      ...
      if (task.timerEnd && now >= task.timerEnd) {
        if (!isTimerExpired) {
          setIsTimerExpired(true); setTimeRemaining('Expired'); updateTaskTimerState(patientId, task.id, true);
    ...
    checkTimer();
    if (task.timerEnd && new Date() < task.timerEnd) { intervalId = setInterval(checkTimer, 30000); }
    return () => { if (intervalId) clearInterval(intervalId); };
  }, [ task.timerEnd, task.id, task.completionStatus, task.isAcknowledged, patientId, patientName, task.text, updateTaskTimerState, isTimerExpired ]);
  ...
  // Event Handlers
  const handleTimerEditSubmit = useCallback(() => { if (!isEditingTimer) return; const minutesToSet = editTimerMinutes.trim() === '' || editTimerMinutes === '0' ? null : editTimerMinutes; updateTaskTimer(patientId, task.id, minutesToSet); setIsEditingTimer(false); }, [isEditingTimer, editTimerMinutes, patientId, task.id, updateTaskTimer]);
  ...
  const handleNotesEditSubmit = useCallback(() => { if (!isEditingNotes) return; updateTaskNotes(patientId, task.id, editNotes); setIsEditingNotes(false); }, [isEditingNotes, editNotes, patientId, task.id, updateTaskNotes]);
  ...
  const handleCompletionToggle = useCallback(() => { const nextStatus: TaskCompletionStatus = task.completionStatus === 'incomplete' ? 'in-progress' : task.completionStatus === 'in-progress' ? 'complete' : 'incomplete'; updateTaskCompletion(patientId, task.id, nextStatus); }, [task.completionStatus, patientId, task.id, updateTaskCompletion]);
  const handleSnooze = useCallback(() => { updateTaskTimer(patientId, task.id, '15'); }, [patientId, task.id, updateTaskTimer]);
  ...
  // Dynamic Styling
  let taskItemClasses = 'flex flex-col py-1.5 group relative';
  let taskTextStyle = 'text-sm flex-1 cursor-default mr-1';
  let timerTextStyle = 'text-xs font-mono whitespace-nowrap';
  if (task.completionStatus === 'complete') { taskTextStyle += ' line-through text-gray-400 dark:text-gray-500'; timerTextStyle += ' text-gray-400 dark:text-gray-500'; }
  else if (isTimerExpired && !task.isAcknowledged) { taskItemClasses += ' animate-flash-bg'; taskTextStyle += ' text-red-700 dark:text-red-400 font-medium'; timerTextStyle += ' text-red-700 dark:text-red-400 font-semibold'; }
  else if (isTimerExpired && task.isAcknowledged) { taskTextStyle += ' text-red-800 dark:text-red-500'; timerTextStyle += ' text-red-800 dark:text-red-500'; }
  else if (task.timerEnd) { taskTextStyle += ' text-gray-800 dark:text-gray-200'; timerTextStyle += ' text-gray-600 dark:text-gray-400'; }
  else { taskTextStyle += ' text-gray-800 dark:text-gray-200'; timerTextStyle += ' text-gray-400 dark:text-gray-600'; }

  return (
    <div className={taskItemClasses}>
      <div className="flex items-center space-x-2 w-full">
        <Button variant="ghost" size="icon" className="h-6 w-6 flex-shrink-0 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700" onClick={handleCompletionToggle} title={`Status: ${task.completionStatus}. Click to change.`}> {getCompletionIcon()} </Button>
        <span className={taskTextStyle} title={task.text}>{task.text}</span>
        <div className="flex items-center space-x-1 flex-shrink-0 ml-auto">
          {isEditingTimer ? ( <> <Input ref={timerInputRef} type="number" min="0" max="999" value={editTimerMinutes} onChange={handleTimerInputChange} onKeyDown={handleTimerInputKeyDown} className="w-14 h-6 text-xs px-1 dark:bg-gray-700" placeholder="Min" /> <Button variant="ghost" size="icon" className="h-6 w-6 text-green-600 hover:text-green-700 dark:text-green-400 dark:hover:text-green-300" onClick={handleTimerEditSubmit} title="Save Timer"> <Save className="h-3.5 w-3.5" /> </Button> <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" onClick={() => setIsEditingTimer(false)} title="Cancel Edit"> <X className="h-3.5 w-3.5" /> </Button> </> )
        ...
        <Button variant="ghost" size="icon" className={`h-6 w-6 ml-1 flex-shrink-0 ${task.notes ? 'text-blue-500 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'} hover:text-blue-600 dark:hover:text-blue-300 opacity-0 group-hover:opacity-100 transition-opacity`} onClick={() => setIsEditingNotes((prev) => !prev)} title={task.notes ? 'Edit/View Notes' : 'Add Notes'}> <MessageSquare className="h-3.5 w-3.5" /> </Button>
        <Button variant="ghost" size="icon" className="h-6 w-6 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" onClick={() => removeTask(patientId, task.id)} title="Remove Task"> <Trash2 className="h-3.5 w-3.5" /> </Button>
      </div>
      {isEditingNotes && ( <div className="mt-1.5 pl-8 pr-2 flex items-start gap-2 w-full"> <textarea ref={notesTextareaRef} value={editNotes} onChange={handleNotesInputChange} onKeyDown={handleNotesKeyDown} placeholder="Add task notes..." rows={2} className="flex-grow text-xs bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded p-1.5 text-gray-800 dark:text-gray-200 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-1 focus:ring-[#008080] focus:outline-none resize-none" /> <Button variant="ghost" size="icon" className="h-6 w-6 text-green-600 hover:text-green-700 dark:text-green-400 dark:hover:text-green-300 self-start flex-shrink-0" onClick={handleNotesEditSubmit} title="Save Notes"> <Save className="h-4 w-4" /> </Button> <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 self-start flex-shrink-0" onClick={() => setIsEditingNotes(false)} title="Cancel Edit"> <X className="h-4 w-4" /> </Button> </div> )}
      {!isEditingNotes && task.notes && ( <div className="mt-1 pl-8 pr-2 text-xs text-gray-600 dark:text-gray-400 italic w-full break-words"> Note: {task.notes} </div> )}
      <div className="pl-8 text-xs text-gray-500 dark:text-gray-500 mt-0.5"> Added: {formatRelative(task.createdAt, new Date())} {task.completionStatus === 'complete' && task.completedAt && ( <span className="ml-2">Completed: {formatRelative(task.completedAt, new Date())}</span> )} </div>
      <style jsx global>{` @keyframes flash-bg { 0%, 100% { background-color: transparent; } 50% { background-color: rgba(239, 68, 68, 0.1); } } .animate-flash-bg { animation: flash-bg 1.5s infinite; } @keyframes pulse-border { 0%, 100% { border-color: ${getBorderColor(300).split(' ')[0]}; } 50% { border-color: ${getBorderColor(300).split(' ')[0].replace('500', '400').replace('600','500')}; } } .animate-pulse-border { animation: pulse-border 1.5s infinite; } `}</style>
    </div>
  );
};
...
const PatientCard: React.FC<PatientCardProps> = ({
  ...
  const handleAddTaskSubmit = useCallback((e?: FormEvent<HTMLFormElement>) => { e?.preventDefault(); if (newTaskText.trim() === '') return; addTaskToPatient(patient.id, newTaskText, newTaskTimerMinutes); setNewTaskText(''); setNewTaskTimerMinutes(''); }, [addTaskToPatient, patient.id, newTaskText, newTaskTimerMinutes]);
  ...
  const handlePatientNotesSubmit = useCallback(() => { if (!isEditingPatientNotes) return; updatePatientNotes(patient.id, editPatientNotes); setIsEditingPatientNotes(false); }, [isEditingPatientNotes, editPatientNotes, patient.id, updatePatientNotes]);
  ...
  return (
    ...
    <Card id={id} className={`mb-4 border-2 ${borderColor} transition-colors duration-500`}>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400" onClick={() => removePatient(patient.id)} title="Remove Patient"> <X className="h-4 w-4" /> </Button>
      </CardHeader>
      <CardContent>
        <div className="text-xs text-gray-600 dark:text-gray-400 mb-3"> <Clock className="inline h-3 w-3 mr-1" /> LOS: <span className="font-semibold text-gray-800 dark:text-gray-200">{lengthOfStayFormatted}</span> <span className="ml-2">(Arrival: {format(patient.arrivalTime, 'HH:mm')})</span> </div>
        <div className="mb-2 flex items-center justify-between"> <div className="text-xs text-gray-700 dark:text-gray-300 font-medium flex items-center"> Notes: <Button variant="ghost" size="icon" className={`h-6 w-6 ml-1 ${patient.notes ? 'text-blue-500 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`} onClick={() => setIsEditingPatientNotes((prev) => !prev)} title={patient.notes ? 'Edit/View Notes' : 'Add Notes'}> <MessageSquare className="h-4 w-4" /> </Button> </div> </div>
        {isEditingPatientNotes && ( <div className="mb-2 flex items-start gap-2 w-full"> <textarea ref={patientNotesTextareaRef} value={editPatientNotes} onChange={(e) => setEditPatientNotes(e.target.value)} onKeyDown={handlePatientNotesKeyDown} rows={2} placeholder="Add patient notes..." className="flex-grow text-xs bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded p-1.5 text-gray-800 dark:text-gray-200 placeholder-gray-500 dark:placeholder-gray-400 focus:ring-1 focus:ring-[#008080] focus:outline-none resize-none" /> <Button variant="ghost" size="icon" className="h-6 w-6 text-green-600 hover:text-green-700 dark:text-green-400 dark:hover:text-green-300 self-start flex-shrink-0" onClick={handlePatientNotesSubmit} title="Save Notes"> <Save className="h-4 w-4" /> </Button> <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 self-start flex-shrink-0" onClick={() => setIsEditingPatientNotes(false)} title="Cancel Edit"> <X className="h-4 w-4" /> </Button> </div> )}
        {!isEditingPatientNotes && patient.notes && ( <div className="mb-3 text-xs text-gray-600 dark:text-gray-400 italic break-words bg-gray-50 dark:bg-gray-700/50 p-2 rounded"> Note: {patient.notes} </div> )}
        ...
        <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
          <form onSubmit={handleAddTaskSubmit} className="flex items-center gap-2">
            <Input type="text" placeholder="Add Task" value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)} onKeyDown={handleNewTaskKeyDown} className="flex-grow h-9 text-sm" />
            <Input type="number" min="1" max="999" placeholder="Min" value={newTaskTimerMinutes} onChange={(e) => setNewTaskTimerMinutes(e.target.value)} onKeyDown={handleNewTaskKeyDown} className="w-16 h-9 text-xs px-1 dark:bg-gray-700 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
            <Button type="submit" variant="ghost" size="icon" className="h-9 w-9 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50" disabled={newTaskText.trim() === ''} title="Add Task"> <Plus className="h-4 w-4" /> </Button>
          </form>
        </div>
      </CardContent>
    </Card>
...
const AddPatientModal: React.FC<AddPatientModalProps> = ({ isOpen, onClose, addPatient }) => {
  ...
  const handleAddTaskLine = useCallback(() => { setTasks(t => [...t, { id: Date.now(), text: '', timerMinutes: '' }]); }, []);
  const handleRemoveTaskLine = useCallback((id: number) => { setTasks(t => { if (t.length <= 1) return [{ id: Date.now(), text: '', timerMinutes: '' }]; return t.filter((task) => task.id !== id); }); }, []);
  const handleTaskChange = useCallback((id: number, field: keyof Omit<ModalTaskState, 'id'>, value: string) => { setTasks(t => t.map((task) => (task.id === id ? { ...task, [field]: value } : task))); }, []);
  const handleSubmit = useCallback((e: FormEvent<HTMLFormElement>) => {
    e.preventDefault(); if (!patientName.trim() || !arrivalTime) return;
    const now = new Date(); let arrivalDateTime = parse(arrivalTime, 'HH:mm', new Date()); arrivalDateTime.setFullYear(now.getFullYear(), now.getMonth(), now.getDate()); if (arrivalDateTime > now) arrivalDateTime = now;
    ...
    addPatient({ id: `patient-${Date.now()}-${Math.random().toString(36).substring(7)}`, name: patientName.trim(), arrivalTime: arrivalDateTime, tasks: processedTasks, notes: patientNotes.trim() });
...
const PatientTrackerSidebar: React.FC = () => {
  ...
  const sidebarWidth = showSidePromptbar ? 'w-80 md:w-96' : 'w-0';
  ...
  return (
    <div className={`flex flex-col h-full overflow-hidden transition-all duration-300 ease-in-out bg-white dark:bg-gray-900 shadow-lg border-l border-gray-200 dark:border-gray-700 ${sidebarWidth}`}>
      {showSidePromptbar && (
        <>
          {/* Sidebar Header */}
          <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0"> <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Patient Tracker</h2> <Button variant="default" size="sm" onClick={() => setIsModalOpen(true)}> <Plus className="h-4 w-4 mr-2" /> Add Patient </Button> </div>
