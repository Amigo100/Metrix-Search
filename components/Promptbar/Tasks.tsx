...
import {
  ...
  ChevronDown,
  ChevronUp,
  PanelLeftClose, // <-- Import icon for collapse toggle
  PanelRightClose, // <-- Import icon for expand toggle
} from 'lucide-react';
import {
  format,
  differenceInMinutes,
...
// --- Mock HomeContext ---
// NOTE: The sidebar's visibility is now primarily controlled by local state below,
// as the mock context always provides showSidePromptbar: true.
...
const PatientTrackerSidebar: React.FC = () => {
  // ***** MODIFICATION START *****
  // Removed context usage for visibility control as it was static mock
  // const { state } = useContext(HomeContext);
  // const { showSidePromptbar } = state;

  // Added local state to control visibility for demonstration
  ...
  // ***** MODIFICATION START *****
  // Use local state 'isLocallyVisible' instead of context 'showSidePromptbar'
  ...
  return (
    <div className={`flex flex-col h-full overflow-hidden transition-all duration-300 ease-in-out bg-white dark:bg-gray-900 shadow-lg border-l border-gray-200 dark:border-gray-700 ${sidebarWidth}`}>
      {/* ***** MODIFICATION START ***** */}
      {/* Use local state 'isLocallyVisible' for conditional rendering */}
      {isLocallyVisible && (
      // ***** MODIFICATION END *****
        <>
          {/* Sidebar Header */}
      ...
      {/* ***** MODIFICATION START ***** */}
      {/* Button to re-open the sidebar if it was closed locally */}
      {!isLocallyVisible && (
         <div className="absolute top-2 left-2 z-10"> {/* Position button */}
             <Button
                 onClick={() => setIsLocallyVisible(true)}
                 variant="ghost"
                 size="icon"
                 className="h-8 w-8 text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 shadow-md"
