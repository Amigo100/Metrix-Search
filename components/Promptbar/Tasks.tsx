...
const AddPatientModal: React.FC<AddPatientModalProps> = ({ isOpen, onClose, addPatientHandler }) => {
  // ... AddPatientModal state and handlers ...
   ...
   const handleSubmit = (e: FormEvent<HTMLFormElement>): void => { /* ... submit logic ... */
        e.preventDefault(); if (!patientName || !arrivalTime) return; const now = new Date(); let arrivalDateTime = parse(arrivalTime, 'HH:mm', new Date()); arrivalDateTime.setFullYear(now.getFullYear(), now.getMonth(), now.getDate()); if (!isValid(arrivalDateTime) || arrivalDateTime > now) { arrivalDateTime = now; }
        const processedModalTasks: Omit<Task, 'id'>[] = tasks .filter((task) => task.text.trim() !== '') .map((task): Omit<Task, 'id'> => { const timerMinutesNum = parseInt(task.timerMinutes, 10); const isValidTimer = !isNaN(timerMinutesNum) && timerMinutesNum > 0 && timerMinutesNum <= 999; const timerEndDate = isValidTimer ? addMinutes(new Date(), timerMinutesNum) : null; return { text: task.text, timerEnd: timerEndDate, isTimerExpired: !!(timerEndDate && timerEndDate <= new Date()), completionStatus: 'incomplete', createdAt: new Date(), completedAt: null, notes: '', isAcknowledged: false }; });
        const newPatientData: Omit<Patient, 'id' | 'tasks'> & { tasks: Omit<Task, 'id'>[] } = { name: patientName, arrivalTime: arrivalDateTime, tasks: processedModalTasks, notes: patientNotes }; addPatientHandler(newPatientData);
      ...
      <DialogContent className="bg-neutral-50 text-black sm:max-w-3xl"> {/* Changed to sm:max-w-3xl */}
       {/* === END OF FIX #1 === */}
        <DialogHeader>
           ...
           <DialogClose asChild> <Button variant="ghost" size="icon" className="absolute top-4 right-4 text-black hover:text-black" onClick={onClose} > <X className="h-4 w-4" /> <span className="sr-only">Close</span> </Button> </DialogClose>
        </DialogHeader>
