// file: /components/Promptbar/Tasks.tsx
...
// Basic shadcn/ui mock components (as in your snippet):
...
const TaskItem: React.FC<TaskItemProps> = ({ task, patientId, updateTaskTimerState }) => {
  ...
  return (
    <div className={`flex items-center space-x-2 py-1 ${taskStyle}`}>
      <Checkbox
        id={checkboxId}
        className="border-gray-500 data-[state=checked]:bg-brand data-[state=checked]:border-brand"
      />
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>): void => {
    ...
    const processedTasks: Task[] = tasks
      ...
      .map((task) => {
        ...
        const isValidTimer =
          !isNaN(timerMinutesNum) && timerMinutesNum > 0 && timerMinutesNum <= 250;
