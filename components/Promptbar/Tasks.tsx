...
const AddPatientModal: React.FC<AddPatientModalProps> = ({ isOpen, onClose, addPatientHandler }) => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>): void => {
    ...
    const processedModalTasks: Omit<Task, 'id'>[] = tasks
      ...
      .map((task): Omit<Task, 'id'> => { // Map to the structure expected by the handler
        ...
        // Calculate timer end relative to NOW when adding, not arrival time
        const timerEndDate = isValidTimer ? addMinutes(new Date(), timerMinutesNum) : null;

        return {
          // id will be generated by the central addPatient handler
          text: task.text,
          timerEnd: timerEndDate,
          // Let the central handler calculate isTimerExpired
          // isTimerExpired: false, // Or calculate based on timerEndDate
          completionStatus: 'incomplete', // Default status
          createdAt: new Date(), // Set creation time to now
          completedAt: null,
          notes: '', // Default notes
          isAcknowledged: false, // Default acknowledged
        };
      });
