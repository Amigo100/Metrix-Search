...
const Button: FC<ButtonProps> = ({
  ...
  const baseClasses =
    'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3D7F80] focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50';
  const sizeClasses = size === 'sm' ? 'h-9 px-3' : 'h-10 px-4 py-2';

  const variants = {
    default:
      'bg-gradient-to-r from-[#2D4F6C] to-[#3D7F80] text-white hover:from-[#254058] hover:to-[#316667] shadow-md hover:shadow-lg ring-offset-stone-50',
    outline:
      'border border-[#3D7F80] bg-white text-stone-700 hover:bg-stone-100 hover:text-stone-900 ring-offset-stone-50',
  };
  const variantClass = variants[variant];

  return (
    <button
      className={cn(baseClasses, sizeClasses, variantClass, className)}
      {...props}
    >
      {children}
    </button>
...
const Input = forwardRef<HTMLInputElement, InputProps>((props, ref) => (
  <input
    ref={ref}
    className="flex h-10 w-full rounded-md border border-[#3D7F80] bg-white px-3 py-2 text-sm text-stone-900 ring-offset-stone-50 placeholder:text-stone-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3D7F80] focus-visible:ring-offset-stone-50 disabled:cursor-not-allowed disabled:opacity-50"
    {...props}
  />
));
...
const Select: FC<SelectProps> = ({
  ...
  const getOptionText = (node: ReactNode): string => {
  ...
  const displayValue = selectedChild
    ? getOptionText(selectedChild.props.children)
    : placeholder;
  ...
  return (
    <div className="relative w-full">
      ...
      <Button
        variant="outline"
        className="w-full justify-between font-normal text-left relative z-0"
        type="button"
...
const Card: FC<CardProps> = ({ children, className = '', ...props }) => (
  <div
    className={cn(
      'rounded-xl border border-stone-200 bg-white text-stone-900 shadow-md transition-shadow duration-200 hover:shadow-lg',
      className
    )}
    {...props}
  >
...
const CardTitle: FC<CardProps> = ({ children, ...props }) => (
  <h3
    className="text-base font-semibold leading-none tracking-tight text-stone-800"
    {...props}
  >
    {children}
  </h3>
...
const PopoverTrigger: FC<PopoverTriggerProps> = ({ children, onClick }) => (
...
const PopoverContent: FC<PopoverContentProps> = ({
  ...
  <div
    className={cn(
      'absolute w-auto p-4 border border-stone-200 rounded-md shadow-lg bg-white z-50 mt-1 space-y-2',
      className
    )}
  >
    {children}
...
const Progress: FC<ProgressProps> = ({
  ...
  >
    <div
      className={`${colorClass} h-full w-full flex-1 transition-transform duration-500 ease-out`}
...
const SwitchMock: FC<SwitchProps> = ({
  ...
  return (
    <button
      ...
      className={cn(
        'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#3D7F80] focus:ring-offset-2 focus:ring-offset-stone-50',
        bgColor
      )}
    >
      <span
...
const PredictiveAnalyticsPage: FC = () => {
  ...
  const generateRecommendations = (results: PredictionResult) => {
    const recs: string[] = [];
    const avgLikelihood =
      (results.wait3h + results.wait4h + results.wait5h + results.wait6h) / 4;
      ...
      <div className="flex flex-col lg:flex-row gap-8 mt-6">
        ...
        <div className="flex-1 lg:pl-8">
          {isLoading && (
            ...
            >
              <div>
                <div className="h-5 bg-stone-300 rounded w-48 mb-4 animate-pulse"></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <Card className="animate-pulse border-stone-200 bg-white">
                    <CardHeader>
                ...
                <div className="h-5 bg-stone-300 rounded w-40 mb-4 animate-pulse"></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  {[...Array(4)].map((_, i) => (
                    <Card
                      key={`skel-breach-${i}`}
                      className="animate-pulse border-stone-200 bg-white"
                    >
                      <CardHeader>
              ...
              <div>
                <div className="h-5 bg-stone-300 rounded w-32 mb-4 animate-pulse"></div>
                <Card className="animate-pulse border-stone-200 bg-white">
                  <CardHeader>
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between">
                        ...
                        <LogIn className="w-5 h-5 text-[#2D4F6C] flex-shrink-0" />
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between">
                        ...
                        <Hourglass className="w-5 h-5 text-[#68A9A9] flex-shrink-0" />
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-3xl font-semibold text-stone-800">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    const { text, color, bgColor, Icon } =
                      formatLikelihood(pred.likelihood);
                    return (
                      <Card key={pred.hours}>
                        <CardHeader>
                          <CardTitle className="flex items-center justify-between">
                ...
                <Card className="bg-white border-stone-200">
                  <CardHeader>
                    <CardTitle className="flex items-center text-stone-800">
                      <Lightbulb className="w-5 h-5 mr-2 text-[#3D7F80]" />
                      Recommendations
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
          ...
          {!isLoading && !predictions && !error && (
            <Card className="mt-6 lg:mt-0">
              ...
              <CardContent className="text-center text-stone-500 space-y-6 px-5 pb-5">
                ...
                />
                <p className="text-sm">
                  Fill in the patient details on the left and click &apos;Predict
                  Wait Time&apos; to see the results displayed here.
                </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <Card className="bg-stone-50 border border-[#3D7F80] shadow-none">
                      <CardHeader>
                        <CardTitle className="text-sm text-stone-600 flex items-center justify-between">
                          <span>Admission Likelihood</span>
                          <LogIn className="w-4 h-4 text-stone-400 flex-shrink-0" />
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                    ...
                    <Card className="bg-stone-50 border border-[#3D7F80] shadow-none">
                      <CardHeader>
                        <CardTitle className="text-sm text-stone-600 flex items-center justify-between">
                          <span>Predicted Wait Time</span>
                          <Hourglass className="w-4 h-4 text-stone-400 flex-shrink-0" />
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        <p className="text-2xl font-semibold text-stone-400">
