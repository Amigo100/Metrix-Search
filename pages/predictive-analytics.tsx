...
interface ApiPredictionInput {
  ...
  occupancy: string; // So the server can use it if needed
}

// =============== Helper UI / Mocked Components ===============
...
const Card: React.FC<CardProps> = ({ children, className = '', ...props }) => (
  <div
    className={cn(
      'rounded-xl border border-slate-200 bg-white text-slate-900 shadow-md transition-shadow duration-200 hover:shadow-lg',
      className,
    )}
    {...props}
  >
...
const CardTitle: React.FC<CardProps> = ({ children, ...props }) => (
  <h3
    className="text-base font-semibold leading-none tracking-tight text-slate-800"
    {...props}
  >
    {children}
  </h3>
...
const PopoverContent: React.FC<PopoverContentProps> = ({ children, className = '' }) => (
  <div
    className={cn(
      'absolute w-auto p-4 border border-slate-200 rounded-md shadow-lg bg-white z-50 mt-1 space-y-2',
      className,
    )}
  >
    {children}
...
const SwitchMock: React.FC<SwitchProps> = ({
  ...
  return (
    <button
      ...
      className={cn(
        'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-50',
        bgColor,
      )}
    >
      <span
...
const PredictiveAnalyticsPage: React.FC = () => {
  ...
  // Updated endpoint to point to /predictive
  ...
  const handlePredict = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      setError(
        'Please fill in all required fields with valid numbers (Age, Triage, Patients Ahead, etc.).',
      );
      setIsLoading(false);
      return;
    }
  ...
  const generateRecommendations = (results: PredictionResult) => {
    ...
    if (results.wait6h > 70 || results.wait5h > 80) {
      recs.push('High risk of extended wait (>5-6h). Consider alerting senior staff.');
    } else if (avgLikelihood > 50) {
      recs.push('Moderate to high likelihood of long wait. Review ED flow if possible.');
    }
    if (avgLikelihood > 40) {
    ...
    if (!isNaN(currentTriageCode) && (currentTriageCode === 1 || currentTriageCode === 2)) {
      recs.push(
        `Patient has high triage acuity (Code ${currentTriageCode}). Ensure prompt assessment.`,
      );
    }
    ...
    if (alteredMentalStatus) {
      recs.push('Note: Patient has altered mental status.');
    }
    if (hasFever) {
      recs.push('Note: Patient presents with fever.');
    }
    if (results.admissionLikelihood > 70) {
      recs.push('High likelihood of admission. Consider notifying inpatient team early.');
    }
    if (results.predictedWaitMinutes > 240) {
      recs.push(
        `Predicted wait time is significant (~${Math.round(
          results.predictedWaitMinutes / 60,
        )} hours). Ensure patient comfort and regular reassessment.`,
      );
    }
    if (recs.length === 0) {
  ...
  const handleDateChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (isValid(newDate)) {
      ...
      const updatedDateTime = setMilliseconds(
        setSeconds(setMinutes(setHours(newDate, currentTime.getHours()), currentTime.getMinutes()), currentTime.getSeconds()),
        ...
        <div className="w-full lg:w-1/3 xl:w-1/4 flex-shrink-0">
          <form onSubmit={handlePredict} className="space-y-5">
                ...
                <PopoverTrigger onClick={toggleCalendar}>
                  <Button
                    ...
                    className={cn(
                      ...
                      !dateTime && 'text-slate-500',
                    )}
                    aria-haspopup="dialog"
                    aria-expanded={isCalendarOpen}
                  >
                    <Clock className="mr-2 h-4 w-4 text-slate-500 flex-shrink-0" />
            ...
            <div>
              ...
              >
                <SelectItem value="1">
                  1 - Immediate threat to life (Immediate)
                </SelectItem>
                <SelectItem value="2">
                  2 - Imminent threat to life (10-mins)
                </SelectItem>
                <SelectItem value="3">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  {[...Array(4)].map((_, i) => (
                    <Card
                      key={`skel-breach-${i}`}
                      className="animate-pulse border-slate-200 bg-white"
                    >
                      <CardHeader>
                      ...
                      <Card key={pred.hours}>
                        ...
                        <CardContent className="space-y-2">
                          <div className="flex justify-between items-baseline mb-1">
                            ...
                            <span
                              className={`text-base font-medium text-slate-500 pr-5`}
                            >
